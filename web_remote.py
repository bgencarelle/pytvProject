...
Endpoints:
  ...
  /diag, /data→ JSON object of diagnostic metrics (CPU, disk, mem, uptime, load, last crash)
  /action?cmd=… → inject control commands (next, prev, toggle, quit)
  /log        → contents of runtime.log (if present)

The server runs in a background thread and auto-restarts on failure,
recording the traceback in the diagnostics.
"""
...
def _overlay_lines(tv: TVEmulator) -> list[str]:
    ch_mgr, ch_num, ref = tv.ch_mgr, tv.curr_ch, tv.ref_time
    phase = tv.phase
    ...
    if chan and chan.files:
        ...
        cur = "PAUSE" if chan.path==PAUSE_SENTINEL else html.escape(os.path.basename(chan.path))
    ...
    if phase == "static":
        lines.append(f"Static burst {_fmt_hms(elapsed)} / {_fmt_hms(config.STATIC_BURST_SEC)}")
...
class RemoteHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _serve_action(self, query: str):
        cmd = urllib.parse.parse_qs(query).get("cmd", [""])[0]
        if   cmd == "next":
...
<script>
 async function refreshUI(){
   try {
     ...
     let txt = ''; for(let [k,v] of Object.entries(dg)){
       txt += k.padEnd(20,' ') + v + '\\n';
     }
