"""
Channel management: discovers video files, reads durations, and computes time offsets.
"""
import os, av
...
class ChannelManager:
    def __init__(self, movies_path):
        self.channels = {}
        files = sorted([f for f in os.listdir(movies_path)
        ...
        for fn in files:
            ...
            try:
                ...
                if vs and vs.duration:
                    ...
                    dur = float(c.duration * c.time_base) if c.duration else 0.0
                c.close()
            except Exception:
                dur = 0.0
    ...
    def next(self, cur):
        if not self.channels: return cur
        nxt = cur + 1
